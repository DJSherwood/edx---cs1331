	...
	while ( flag ) {
		...
		switch (chosenOp) {
			...
			case "multiply":
				...
				if ( progInput.hasNextDouble() ) {
					double mul1 = progInput.nextDouble();

					if ( progInput.hasNextDouble() ) {
						double mul2 = progInput.nextDouble();
		        			douAns = mul1 * mul2;
			...
			case "divide":
				...
				System.out.printf("Enter two doubles:\n");
				if ( progInput.hasNextDouble() ) {
					double div1 = progInput.nextDouble();

					if ( progInput.hasNextDouble() ) {
						double div2 = progInput.nextDouble();
